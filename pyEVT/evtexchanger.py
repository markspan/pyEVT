...
"""
    Description: Library for interacting with a USB device using the libopensesame and hid libraries.
"""
...
class EvtExchanger:
    """
    Description: Class for managing interaction with a USB device.
    """
    SelectedDevice = None
    ListOfDevices = []
    Path = []
    ...
    def Attached(self, matchingkey="EventExchanger"):
        """
        Description: Initialize the USB device and populate the list of available devices.
        """
    ...
    def Select(self, deviceName):
        """
        Description: Select a specific device from the available devices.
        """
    ...
    def WaitForDigEvents(self, AllowedEventLines, TimeoutMSecs):
        """
        Description: Wait for digital events with a specified timeout.
        """
        # Flush the buffer
    ...
    def GetAxis(self):
        """
        Description: Get the axis value from the USB device.
        """
        while self.SelectedDevice.read(1) != []:
            pass
        time.sleep(0.01)
    ...
    def PulseLines(self, OutValue, DurationInMillisecs):
        self.SelectedDevice.write([0, self.__PULSEOUTPUTLINES, OutValue,
                                  DurationInMillisecs & 255, DurationInMillisecs >> 8, 0, 0, 0, 0, 0, 0])

    def SetAnalogEventStepSize(self, NumberOfSamplesPerStep):
        self.SelectedDevice.write(
            [0, self.__SETANALOGEVENTSTEPSIZE, NumberOfSamplesPerStep, 0, 0, 0, 0, 0, 0, 0, 0])

    def RENC_SetUp(self, Range, MinimumValue, Position, InputChange, PulseInputDivider):
        self.__AxisValue = Position
        self.SelectedDevice.write([0, self.__SETUPROTARYCONTROLLER, Range & 255, Range >> 8, MinimumValue &
                                  255, MinimumValue >> 8, Position & 255, Position >> 8, InputChange, PulseInputDivider, 0])

    def RENC_SetPosition(self, Position):
        self.__AxisValue = Position
        self.SelectedDevice.write(
            [0, self.__SETROTARYCONTROLLERPOSITION, Position & 255, Position >> 8, 0, 0, 0, 0, 0, 0, 0])

    def SetLedColor(self, RedValue, GreenValue, BlueValue, LedNumber, Mode):
        self.SelectedDevice.write(
            [0, self.__SETWS2811RGBLEDCOLOR, RedValue, GreenValue, BlueValue, LedNumber, Mode, 0, 0, 0, 0])

    def SendColors(self, NumberOfLeds, Mode):
        self.SelectedDevice.write(
            [0, self.__SENDLEDCOLORS, NumberOfLeds, Mode, 0, 0, 0, 0, 0, 0, 0])
